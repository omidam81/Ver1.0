...
var dagFiles = {
	tree:{
		...
		create: function(e){
			var person	= prompt("Create a New Folder", "new-folder");
			if (person!=null && person!="")
			{			
				var str 	= $d(e).attr('rel');
				var data 	= str.split(':/');
				var url = base + 'index.php?option=com_devn_vmattribute&view=ajax&task=media&tmpl=ajax&type='+data[0]+'&action=createFolder';
				$d.post(url, { folder: data[1], add: person })
				.done(function(data){
					if(data == '1')
					{
						dagFiles.tree.load(e);
					}
				});
			}
			if(person == ''){alert('Please enter folder name');}
		}
	},
	file: {			
		edit: function(path, e){
			...
			if(title != '' && title != null)
			{
				var url = base + 'admin/media/filefename';				
		...
		remove: function(path, e){
			...
			var url = base + 'admin/media/fileremove';
	...
	ajax:{
		folder: function(e){	
			...
			var url = base + 'admin/media/folder';
	...
	folder:{
			...
			$d.post(url, { path: path }).done(function(content) {
				...
				if(data.folder.length > 0)
				{
					for(i=0; i<data.folder.length; i++)
					{
						...
						var html = '<img alt="css" src="'+base_url+'assets/images/folder-icon-67X67.png">' + '<span class="file-name">' + data.folder[i] + '</span>';
				...
				if(data.files.length > 0)
				{
					for(i=0; i<data.files.length; i++)
					{
						...
						{
							var html = '<img alt="'+data.files[i].name+'" src="'+ base_url + 'assets/images/file/' + data.files[i].exten + '.png">' + '<span class="file-name">' + data.files[i].title +'</span>';
						}
		...
		rename: function(src, name, e){
			...
			if(typeof e == 'undefined' || e == false)
			{
				...
				if(title != null)
				{
					var url = base + 'admin/media/rename';
		...
		remove: function(src, o, load){
			...
			else
			{
				...
				var url = base + 'admin/media/remove';
