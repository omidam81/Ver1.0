       ...
       [HttpPost]
        public ActionResult SendMail(string accountNumber, string bankName, string accHoldName, string contNum, string messAdmin)
        {
            ...
            if (balance > 0)
            {
                ...
                _paymentInfService.AddPayment(new PaymentInformationRecord { AccountNumber = Convert.ToInt32(accountNumber), AccountHolderName = accHoldName, BankName = bankName, ContactNumber = Convert.ToInt32(contNum), MessAdmin = messAdmin, TranzactionId = payout.Id});
