...
{
    public class SendFinishedCampaignafter1DayToSellerTaskHandler : IScheduledTaskHandler
    {
        private const string TASK_TYPE = "SendFinishedCampaignafter1DayToSellerTask";
        ...
        public SendFinishedCampaignafter1DayToSellerTaskHandler(IScheduledTaskManager taskManager, ICampaignService campaignService, ITeeyootMessagingService teeyootMessagingService)
        {
        ...
        public void Process(ScheduledTaskContext context)
        {
            if (context.Task.TaskType == TASK_TYPE)
            {
                try
                {
                    ...
                    _teeyootMessagingService.SendCampaignFinished1DayMessageToSeller();
                }
                catch (Exception e)
                {
